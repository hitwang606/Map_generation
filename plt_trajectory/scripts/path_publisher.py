...
if __name__ == '__main__':
    ...
    marker_pub1 = rospy.Publisher('/odom/robot1', Marker, queue_size=10)
    marker_pub2 = rospy.Publisher('/odom/robot2', Marker, queue_size=10)
    marker_pub3 = rospy.Publisher('/odom/robot3', Marker, queue_size=10)
    rate = rospy.Rate(10)  # 1 Hz
    
    script_dir = os.path.dirname(__file__)
    file_path = os.path.join(script_dir, '__path__.npy')
    # 从参数服务器获取矩阵参数
    drone_id = rospy.get_param('~ID')
    path_array = np.load(file_path)
    marker_vec1 = path_array[(drone_id-1)*3:drone_id*3,199:200].reshape(3,1)  # 转置以适应3xN格式
    marker_vec2 = path_array[(drone_id-1)*3:drone_id*3,349:350].reshape(3,1)  # 转置以适应3xN格式
    marker_vec3 = path_array[(drone_id-1)*3:drone_id*3,499:500].reshape(3,1)  # 转置以适应3xN格式
