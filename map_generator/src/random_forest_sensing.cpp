...
void RandomMapGenerateMRAC1() {
  ...
  //generate constant polar obs


  float positionobs[20][2] =
	{
		{10.0,0.8},
		{10.0,2.5},
		{10.0,-0.8},
  ...
  float radiusobs[20]={0.35,0.6,0.30,0.5,0.65,0.65,0.60,0.6,0.60,0.60,0.20,0.25,0.10,0.20,0.20,0.25,0.20,0.15,0.20,0.25};
  ...
  for (int i = 0; i < 20 && ros::ok(); i++) {
    ...
    double radius = radiusobs[i];
    int widNum = 2*ceil((radius) / _resolution);
...
void RandomMapGenerateMRAC_EXP_Cylinder() {
  pcl::PointXYZ pt_random;
  ...
  float radiusobs[5]={0.18,0.18,0.18,0.18,0.18};
...
void RandomMapGenerateMRAC_EXP_Column() {
  pcl::PointXYZ pt_random;
...
void RandomMapGenerateMRAC2() {
  ...
  float radiusobs[20]={0.4,0.6,0.30,0.6,0.8,0.70,0.270,0.7,0.7,0.7,0.3,0.3,0.2,0.3,0.3,0.3,0.3,0.2,0.3,0.3};
...
void saveMapToPCD() {
  if (cloudMap.points.empty()) {
    ROS_WARN("Map is empty, skip saving PCD file.");
    return;
  }

  std::string filename = "/home/ym/catkin_ws/src/map_generator/random_map.pcd"; // 你可以修改这个路径
  ROS_INFO("Saving map to %s ...", filename.c_str());
  
  try {
    pcl::io::savePCDFileASCII(filename, cloudMap);
    ROS_INFO("Saved %ld data points to %s.", cloudMap.points.size(), filename.c_str());
